...
import { handleOaiIfMatch } from "./oaiHandler";
...
export default {
    async fetch(request: Request, env: Env): Promise<Response> {
        ...
        // Handle OpenAI-compatible passthrough with provider config
        const oaiResponse = await handleOaiIfMatch(request, env);
        if (oaiResponse) {
            return oaiResponse;
        }

        if (url.pathname === "/ant/v1/messages" && request.method === "POST") {
